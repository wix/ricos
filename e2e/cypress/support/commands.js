...
Cypress.Commands.add('addHtmlPlugin', (input, data) => {
  cy.get(`[data-hook*=${input}]`)
    .click()
    .clear();
  cy.get(`[data-hook*=${input}]`).typeAllAtOnce(data);
...
Cypress.Commands.add('addUrl', () => {
  ...
  cy.get(`[data-hook*=${HTML_PLUGIN.URL}]`).click();
  cy.addHtmlPlugin(HTML_PLUGIN.URL_INPUT, 'https://www.wix.com');
});
