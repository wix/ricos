  ...
  originalFn({
    ...
    scriptHooks: {
      beforeCaptureScreenshot:
        "[...document.styleSheets].forEach(s => [...s.rules].forEach(r => r.style && r.style.getPropertyValue('font-family') && r.style.setProperty('font-family', r.style.getPropertyValue('font-family').split(',').map(f => f.trim().includes('Helvetica') ? 'sans-serif' : f).join(','))))",
    },
  });
});
