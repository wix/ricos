...
| `disableExpand` (deprecated!) | disables expanding images & fullscreen | false         | No           | viewer (this option is deprecated! if used it will be supported and the same behavior will remain. note that now the initial value needs to be set in the editor settings)           |
