...
```jsx
onChange?: (content: RicosContent | RichContent) => void
```
...
Determines whether the editor should supply content in the old `draft-js` schema or the new `RichContent` schema.
Affects the return value of the differnet output methods (`onChange`, `getContent`, `getContentPromise`).
