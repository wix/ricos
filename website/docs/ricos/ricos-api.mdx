...
:::note
For rendering a viewer, import the plugin from the viewer entry point <br/> `ricos/video/viewer`
:::
...
Container width, when applicapble, will be used to improve performance (prevent client side measurments)
...
| `getContent(): RicosContent`                                                                    | Retrieves current content         |
| `getContentPromise(options: { flush?: boolean; publishId?: string}): Promise<RicosContent>`     | Retrieves current content promise |
