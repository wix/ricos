...
```jsx
`Note disableRightClick?: boolean; is deprecated`
...
#### `disableRightClick` (deprecated) if used the same behavior will remain

Disables the mouse right click functionality for the gallery, video and image plugins.

#### `disableDownload`

Disables the mouse right click functionality for the gallery, video and image plugins and also sets the initial download option for custom videos.
