...
The `pluginButtons` parameter is an `inline-button` data array (see divider plugin [inline-buttons](https://github.com/wix/ricos/blob/master/packages/plugin-divider/web/src/toolbar/inline-buttons.tsx) for reference).
...
All the toolbar types are exposed by the `TOOLBARS` const found in [consts.js](https://github.com/wix/ricos/blob/master/packages/editor-common/web/src/consts.js).
...
All available button names are listed in the [EditorPlugins](https://github.com/wix/ricos/blob/master/examples/main/shared/editor/EditorPlugins.jsx) file under the `getToolbarSettings` section.
...
| `getTextPluginButtons`   | a map of inline buttons added by plugins. The keys are derived from the `PluginTextButtonMappers` -- see the `link-plugin`'s [createLinkToolbar](https://github.com/wix/ricos/blob/master/packages/plugin-link/web/src/toolbar/createLinkToolbar.js) for reference | text editing toolbars                       |

#### Display Options

At the moment, the `getDisplayOptions` API consists of a single property `displayMode`. This property accepts two values (defined in [consts.js](https://github.com/wix/ricos/blob/master/packages/editor-common/web/src/consts.js)):
...
The [default-toolbar-settings.js](https://github.com/wix/ricos/blob/master/packages/editor/web/src/RichContentEditor/Toolbars/default-toolbar-settings.js) contains the default toolbar settings, and the `getToolbarSettings` code example could be found in [PluginConfig.js](https://github.com/wix/ricos/blob/master/examples/editor/src/PluginConfig.js) (commented by default).
