...
```tsx
import { fromHtml } from 'ricos-content/libs/htmlConverters';
import { RichContent } from 'ricos-schema';
import { RicosEditor, DraftContent } from 'ricos-editor';
...
The following functions allow converting rich content to and from ProseMirror.

### HTML to rich content

```tsx
fromProseMirror: (content: ProseContent) => RichContent;
fromProseMirror: (node: ProseContent) => Node; // Convert a single node
fromProseMirror: (data: Record<string, any>) => Record<string, any>; // Convert a node's data
```
...
const content: RichContent = fromProseMirror(proseContent);
const draftContent: DraftContent = toDraft(content);
...
```tsx
toProseMirror: (content: RichContent) => ProseContent;
toProseMirror: (node: Node) => ProseContent; // Convert a single node
toProseMirror: (data: Record<string, any>) => Record<string, any>; // Convert a node's data
```

#### Example

```tsx
import { toProseMirror } from 'ricos-content/libs/converters';
import ricosContent from './ricosCotnent.json';

const content: ProseContent = toProseMirror(ricosContent);
```
