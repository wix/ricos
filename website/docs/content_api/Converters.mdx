...
```tsx
toPlainText: (content: RichContent, options?: PlainTextOptions) => Promise<string>;
```
...
The following functions provide a utility for converting rich content to and from HTML.
