...
export default class ErrorToast extends Component {
  ...
  createErrorMap = () => {
    return {
      ...
      [MediaUploadErrorKey.IMAGE_DIMENSIONS_EXCEEDED]:
        'UploadImage_Error_Image_Dimensions_Exceeded',
      ...
      [MediaUploadErrorKey.MP3_UNSUPPORTED_STEREO]:
        'UploadAudio_Error_MP3_Unsupported_Stereo_Format',
      ...
      [MediaUploadErrorKey.VIDEO_DURATION_MISMATCH]:
  ...
  getErrorMessage = () => {
    ...
    if (!this.errorMap) {
      this.errorMap = this.createErrorMap();
    }
    const translationKey =
      errorCount > 1 ? 'UploadFile_Error_Generic_Toast_Multiple' : this.errorMap[error.key];
