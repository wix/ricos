import { Node, VideoSource } from 'ricos-schema';
...
const getListSymbol = (index: number, type: Node.Type) =>
  type === Node.Type.ORDERED_LIST ? index + 1 + '. ' : '• ';

const parseList = (listNode: Node): { type: Node.Type; item: string }[] =>
  listNode.nodes
