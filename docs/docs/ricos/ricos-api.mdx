...
:::note
- Please set `forPublish=true` so Ricos can trigger our publish BI event.
- Please do so **only when relevant**, meaning: when you use `getContent` for your own publish functionality.
:::
...
class MyComponent extends React.Component {
  publishPost(postId) {
    const content = this.editor.getContent(postId, true); //'true' triggers our publish BI
    
    //here comes your own implementation of publish...
  }
}
```
### `isEditor`: boolean
