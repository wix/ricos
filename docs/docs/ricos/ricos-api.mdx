...
### `previewSettings`
Activates a preview decoration: a shortened version of a content, ready to be expanded.
Preview props are aligned with the legacy [RichContentPreview](https://github.com/wix-incubator/rich-content/blob/master/docs/docs/rcp_api/rich-content-preview.md) props.
```jsx
previewSettings?: {
...
render() {
  ...
  return (
    <RicosViewer
      content={content}
      previewSettings={{ interactionMap, defaultTransformation }}
    />
  );
}
```