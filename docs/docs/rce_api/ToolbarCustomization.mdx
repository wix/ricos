...
The `pluginButtons` parameter is an `inline-button` data array (see divider plugin [inline-buttons](https://github.com/wix/rich-content/blob/master/packages/plugin-divider/web/src/toolbar/inline-buttons.tsx) for reference).
...
All the toolbar types are exposed by the `TOOLBARS` const found in [consts.js](https://github.com/wix/rich-content/blob/master/packages/editor-common/web/src/consts.js).
...
All available button names are listed in the [EditorPlugins](https://github.com/wix/rich-content/blob/master/examples/main/shared/editor/EditorPlugins.jsx) file under the `getToolbarSettings` section.
...
| `getTextPluginButtons`   | a map of inline buttons added by plugins. The keys are derived from the `PluginTextButtonMappers` -- see the `link-plugin`'s [createLinkToolbar](https://github.com/wix/rich-content/blob/master/packages/plugin-link/web/src/toolbar/createLinkToolbar.js) for reference | text editing toolbars                       |

#### Display Options

At the moment, the `getDisplayOptions` API consists of a single property `displayMode`. This property accepts two values (defined in [consts.js](https://github.com/wix/rich-content/blob/master/packages/editor-common/web/src/consts.js)):
...
So, the `getToolbarDecorationFn` comes to rescue. This function is expected to return a `Component` to be rendered instead of the default toolbar container. This `Component` will be provided with the toolbar container props. For reference, check out the [InlineToolbarDecoration.jsx](https://github.com/wix/rich-content/blob/master/examples/main/src/Components/InlineToolbarDecoration.jsx)

## Plugin functionality toolbar customization

The `getButtons` property, when applied on `TOOLBARS.PLUGIN`, will affect ALL the plugin functionality toolbars. This can be used, for example, to hide size-related buttons for _all_ the plugin toolbars.
...
The [default-toolbar-settings.js](https://github.com/wix/rich-content/blob/master/packages/editor/web/src/RichContentEditor/Toolbars/default-toolbar-settings.js) contains the default toolbar settings, and the `getToolbarSettings` code example could be found in [PluginConfig.js](https://github.com/wix/rich-content/blob/master/examples/editor/src/PluginConfig.js) (commented by default).
